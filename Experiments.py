...
if available_gpus == 0:
    raise RuntimeError("No GPU detected. Please ensure CUDA is installed and a GPU is available.")
GPU_IDs = list(range(available_gpus))
print(f"{GENERAL_INFO} Detected {available_gpus} GPU(s): {GPU_IDs}")
...
def main(is_FL, TEST_CASE_ID=1, PARAM_ID=0):
    ...
    for i, client_config in enumerate(clients_configs):
        ...
        python_command = (
            ...
            f"clients_configs = get_clients_configs(TEST_CASES[{TEST_CASE_ID}]);"
            f"params = get_hyperparameters(clients_configs[{i}].get('model_name'), {is_FL});"
            f"params[{PARAM_ID}].device = 'cuda:{gpu_id}';"
...
if __name__ == "__main__":
    for is_FL in [True]:
        for TEST_CASE_ID in [0]:
            ...
            for i in range(len(parmas)):
                print(f"{GENERAL_INFO} TEST CASE {TEST_CASE_ID}: {model_name} -- "
                      f"{'CENTRALIZED LEARNING' if is_FL == False else f'FL SCHEME: {parmas[i].FL_scheme}'} -- "
                      f"CSV: {parmas[i].use_CSV}")
                main(is_FL, TEST_CASE_ID=TEST_CASE_ID, PARAM_ID=i)
