...
---
Thien Pham, Oct 2024
Modified for GPU Setup, July 2025
"""
...
from FL_HELPERS.FL_constants import SERVER_INFO, CLIENT_INFO_TRAINING, DATA_INFO_VERBOSE
...
def run_server(params, num_clients, is_FL=True):
    ...
    Args:
        ...
        is_FL (bool, optional): Whether to run Federated Learning or not.
    """

    # Use GPU if available
    if torch.cuda.is_available():
        params.device = "cuda:0"
        print(f"{SERVER_INFO} Using GPU: {torch.cuda.get_device_name(0)}")
    else:
        params.device = "cpu"
        print(f"{SERVER_INFO} Using CPU")
...
def run_client(params, client_config, is_FL=True, device=None):
    ...
    Args:
        ...
        client_config (dict): Configuration for the client.
        is_FL (bool, optional): Whether to run Federated Learning or not.
        device (str): Specific device to use (e.g., 'cuda:0').
    Returns:
        FL_Client: The client object.
    """
    model_name = client_config.get('model_name')
    ...
    if torch.cuda.is_available():
        if device:
            ...
            params.device = "cuda:0"
        print(f"{CLIENT_INFO_TRAINING} {client_name} Using GPU: {torch.cuda.get_device_name(int(params.device.split(':')[1]))}")
    else:
        params.device = "cpu"
        print(f"{CLIENT_INFO_TRAINING} {client_name} Using CPU")
    ...
    if model_name == 'FedConvLSTM':
        ...
        # Generate the data
        X_train, Y_train, X_test, Y_test, scaler = generate_data(
            OD_tensor_client, 
            None, 
            params.lookback, 
            params.ahead, 
    ...
    elif model_name in ['Fed-LSTM-DSTGCRN', 'FedGRU', 'FedLSTM', 'FedARIMA', 'FedLR', 'FedAGCRN']:
        if "OD" not in data_path:
            (train_loader, val_loader, test_loader, scaler, num_nodes, num_features, num_obsevations) = get_dataloader(
                data_path,
                params,
